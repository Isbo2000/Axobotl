...
module.exports = {
    ...
    callback: async (message, arguments, text) => {
        ...
        const background = await Canvas.loadImage('./assets/imagecommands/crttv/crttv-bg.png');
        const foreground = await Canvas.loadImage('./assets/imagecommands/crttv/crttv-fg.png');
        ...
        if (message.attachments?.size == 1) {
            ...
            if(member) {
                if (member.avatar) {
                    const image = await Canvas.loadImage("https://cdn.discordapp.com/avatars/"+member.id+"/"+member.avatar+".png")
                    context.drawImage(image, 85, 102.5, 255, 200);
