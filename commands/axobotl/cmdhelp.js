...
module.exports = {
    ...
    callback: (message, arguments, text) => {
        ...
        gencmd = gencmd.join(',')
        gencmd = gencmd.replace(/[,]/g, function($1) { return $1 === ',' ? '' : ',' })
        ...
        imgcmd = imgcmd.join(',')
        imgcmd = imgcmd.replace(/[,]/g, function($1) { return $1 === ',' ? '' : ',' })

        modcmd = [
            ("`" + prefix + 'clear` | clears the last 49 messages(yes its exactly 49)\n'),
            ("`" + prefix + "kick <user> <reason>` | kicks the specified user from the server\n"),
            ("`" + prefix + "ban <user> <reason>` | bans the specified user from the server\n"),
            ("`" + prefix + "unban <userID>` | unbans the user specified from their user ID\n"),
            ("\u200B\n")
        ]
        modcmd = modcmd.join(',')
        modcmd = modcmd.replace(/[,]/g, function($1) { return $1 === ',' ? '' : ',' })
